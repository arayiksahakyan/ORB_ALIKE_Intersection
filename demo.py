...
if __name__ == '__main__':
    ...
    for img in progress_bar:
        ...
        # run model to get dense score map only
        #dense = model.extract_dense_map(img_rgb, ret_dict=True)
        img_t = torch.from_numpy(img_rgb).float() / 255.0  # convert to float tensor 0-1
        img_t = img_t.permute(2, 0, 1).unsqueeze(0)        # (H,W,3) -> (1,3,H,W)
        img_t = img_t.to('cpu')                            # ensure CPU
        dense = model.extract_dense_map(img_t, ret_dict=True)
        score_map = dense['score_map'][0, 0].detach().cpu().numpy()  # -> shape (H, W)

        heatmap = (score_map * 255).astype('uint8')
        heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)

        # display image
        out = heatmap

        runtime.append(1e-3)  # fake time data so FPS works
        N_matches = 0
        kpts = []  # no keypoints
        ...
        status = f"Fps:{ave_fps:.1f}  (Heatmap)"
